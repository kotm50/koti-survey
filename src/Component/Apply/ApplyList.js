...
function ApplyList() {
  ...
  const getList = async e => {
    const querySnapshot = await getDocs(
      query(collection(db, "apply"), orderBy("created", "desc"), limit(10))
    );
    let list = [];
  ...
  const searchIt = async e => {
    ...
    let searchSnapshot;
    if (keyword !== "" && phoneNum === "") {
      searchSnapshot = await getDocs(
        query(collection(db, "apply"), where("name", "==", keyword), limit(10))
      );
    } else if (keyword !== "" && phoneNum !== "") {
      searchSnapshot = await getDocs(
        query(
          collection(db, "apply"),
          where("name", "==", keyword),

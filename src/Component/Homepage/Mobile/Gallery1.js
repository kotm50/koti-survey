...
function Gallery() {
  ...
  const fetchData = async page => {
    const perPage = 10;
    const q = query(collection(db, "gallery1"), orderBy("id", "desc"));
  ...
  const handleNextPage = () => {
    if (page < totalPages) {
      navi(`/mobile/gallery1?page=${page + 1}`);
    }
  };
  ...
  const handleLastPage = () => {
    if (page < totalPages) {
      navi(`/mobile/gallery1?page=${totalPages}`);
    }
  };

  const handleFirstPage = () => {
    if (page > 1) {
      navi(`/mobile/gallery1?page=${1}`);
    }
  };
    ...
    <>
      ...
      <div className="mt-[45px] mb-[16px]">
        <h2 className="text-[30pt] font-bold text-center">
          <span className="text-kotired">I</span>NTERVIEW
        </h2>
      ...
      {galleryData.length > 0 ? (
        <div className="px-[20px]">
          <div className="border-b-2 border-gray-500 flex flex-row justify-between mb-[16px] items-center">
          ...
          <div className="grid grid-cols-1 gap-y-0">
            {galleryData.map((gal, idx) => (
              <Link
                to={`/mobile/gallerydetail?id=${gal.id}&tag=1`}
                key={gal.id}
                className="group"
              >
